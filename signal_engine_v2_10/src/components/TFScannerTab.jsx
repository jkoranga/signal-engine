...
export default function TFScannerTab({ timeframe, tabColor, settings, update, user, isFirstVisit, isActive, onAlertCount, onScanProgress }) {
  ...
  const activeScanners = useMemo(() => {
    ...
    return ALL_SCANNERS.filter(s => {
      ...
      // No user override — use the scanner's own declared tfs[] as default.
      // If scanner has no tfs defined, run on all TFs as a last resort.
      const defaultTfs = s.tfs && s.tfs.length > 0 ? s.tfs : null
      return defaultTfs ? defaultTfs.includes(timeframe) : true
    })
